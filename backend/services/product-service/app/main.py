...
# In-memory database for products (will be replaced with MongoDB)
products_db = []
categories_db = []
reviews_db = []
...
class ProductCreate(BaseModel):
    ...
    price: Dict[str, float]  # {"amount": 150000, "currency": "KES"}
    category: List[str]
...
def create_product(product: ProductCreate):
    ...
    product_dict["seller_id"] = None  # Will be set when auth is integrated
...
def create_review(product_id: str, review: ReviewCreate):
    ...
    review_dict["user_id"] = "temp_user"  # Will be set when auth is integrated
